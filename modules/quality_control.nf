...
process CUTADAPT_TRIM_COMPLEMENT {
    label "container_qiime2"
    publishDir "${params.outdir}/stats/trim/", pattern: "*.log"
    tag "${set_id}"
    ...
    output:
    tuple val(set_id), path("complement_trimmed_${set_id}_seqs.qza"), emit: qza
    path "*.log",                                          emit: stats
    ...
    if (params.read_type == "single") {
        ...
        qiime cutadapt trim-single \
            ...
            --p-quality-cutoff-3end ${params.cutadapt.quality_cutoff_3end} \
            --p-quality-base ${params.cutadapt.quality_base} \
            --o-trimmed-sequences complement_trimmed_${set_id}_seqs.qza \
            --verbose | tee complement_trimmed_${set_id}_stats.log        """
    } else if (params.read_type == "paired") {
        """
        ...
        qiime cutadapt trim-paired \
            ...
            --p-quality-base ${params.cutadapt.quality_base} \
            --o-trimmed-sequences complement_trimmed_${set_id}_seqs.qza \
            --verbose | tee cmplement_trimmed_${set_id}_stats.log        """
    }
}